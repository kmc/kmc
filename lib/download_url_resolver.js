...
DownloadUrlResolver.resolve = function(url) {
  if (this.isMediafire(url)) {
    return resolveMediafireUrl(url);
  } else if (this.isDropbox(url)) {
    return resolveDropboxUrl(url);
  } else if (this.isCurseforge(url)) {
    return resolveCurseforgeUrl(url);
  } else if (this.isBox(url)) {
    return resolveBoxUrl(url);
  } else {
    // no special rules are known about this URL ... simply give the URL back
    // (in a promise).
...
var resolveMediafireUrl = function(url) {
