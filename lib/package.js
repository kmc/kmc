...
// Performs the installation procedure for this package.
//
// The parameter `tempDir` should be a string to the temporary location where
// the already downloaded and decompressed package contents can be found.
//
// `kspDir` should be the KSP directory to install into.
Package.prototype.performInstallProcedure = function(tempDir, kspDir) {
  // Substrings which, if found in the file or directory to be copied, will
  // cause merge_directory to ignore the file / directory.
  var mergeDirectoryExcludeSubstrs = ['000_Toolbar', 'ModuleManager'];
  var mergeDirectoryFilterFn = function(name) {
    return !mergeDirectoryExcludeSubstrs.some(function(pattern) {
      return name.indexOf(pattern) > -1;
    });
  }

  var handleMergeDirectory = function(args) {
    var src = path.join(tempDir, args[0]);
    var dest = path.join(kspDir, args[1] || '.', args[0]);

    return fse.copyAsync(src, dest, mergeDirectoryFilterFn);
  }

  // The approach here is to map each command to a promise to perform that
  // action, then return a promise fulfilled when all the individual promises
  // are fulfilled.
  //
  // The 'concurrency' option passed to `map` ensures that the promises are
  // performed sequentially ("1" at a time).
  return Promise.map(this.installProcedure, function(command) {
    var commandName = Object.keys(command)[0];

    if (commandName === 'merge_directory') {
      return handleMergeDirectory(command[commandName]);
    }
  }, {concurrency: 1});
}
